...
public class TaxProgram {
    ...
    public static void main(String[] args) {
        ...
        // Compute tax and set tax variable to zero to avoid compile error
        double tax = 0;
        
        // Compute for single filers
        if (status == 0) {
            if (income <= 8350)
                tax = income * 0.10;
        ...
        // Compute for married file jointly or qualifying widow(er)
        else if (status == 1) {
            if (income <= 16700)
                tax = income * 0.10;
        ...
        // Compute tax for married separetly
        ...
        // exit program if user enters invalid status
        else {
